...
| 键尾剥「（≤18 位字母数字）」码（#82：6→18） | `贸易方式（0110）` / `境内收货人（91440300MA5FETXT25）` | `贸易方式` / `境内收货人` |
...
进境备案清单把码印在**标签**里：「境内收货人（91440300MA5FETXT25）」。`trailing_code` 字段（tradeName / ownerName）在值侧发射后，检查 KV 键尾的括号码：

| 键尾码 | 落点 | 证据 |
|---|---|---|
| 18 位信用代码 | `scc_target`（tradeScc / ownerScc） | 标签格 |
| 10 位海关码 | `split_target`（tradeCode / ownerCode） | 标签格 |

值侧已拆出同一目标时不重复写。KV 键保留格子原文（剥码只发生在匹配侧）。
...
| 键带空白 / 换行 / 全角括号 / 尾码（`贸易方式（0110）`、`境内收货人（USCC）`） | 已被键归一化吃掉，不用管 | 否 |
...
| 表尾声明区（特殊关系确认 / 报关人员…）进表体 | `layout_vocab.yaml` `table_footer_tokens` | 否 |
| 新的「标签（码）」写法剥不掉 | `textnorm.py` 剥码正则 | 是，常量 |
| 值是纯码 / 括号码，新形状 | `head_map.py` 值路由规则 | 是，通用规则 |
| OCR 日期是紧凑 `YYYYMMDD` 没过值域 | 已收（#82）；新日期形状改 `layout.py` 值域 | 是，一处 |
| OCR 标签与值差一列没配上 | 已收邻列兜底（#82）；漂 >1 列改 `layout.py` `_adjacent_below` | 是，一处 |
