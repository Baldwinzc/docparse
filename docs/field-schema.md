...
本目录只回答「输出什么、从哪类版面来」。名称转 code 见 [code-tables.md](code-tables.md)（#14），BOX/TABLE 别名见 [layout-vocab.md](layout-vocab.md)（#13），表头映射见 [head-map.md](head-map.md)（#17），商品映射见 [goods-map.md](goods-map.md)（#18），整单组装见 [assemble.md](assemble.md)（#19），抽完查什么见 [validate-rules.md](validate-rules.md)（#20，待业务确认）。
