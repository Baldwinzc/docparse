...
| 表头孤儿续行 | 没有上一件可并 → 丢弃 |
| 无身份值行（#84） | 没有项号 / 税号 / 品名、但有数量 / 单价 / 币制值（标准报关单规格单行的 item 行C）→ 有项号列才并上一件 |
| 同名字段 | 主行空位直接补，已占用不覆盖 |
| 跨字段 | 续行值落在已占 `gname`：规格形状（归一化后含 `\|` 或 `%`，全角 `｜` `％` 算）或 `gmodel` 已有值 → 并 `gmodel`，硬换行无缝拼接（#84） |
...
| `keep`（默认） | 整格进该字段 |
| `skip` | 本层不映射 |
| `leading_hs` | 列值前缀 8–10 位数字进 `codeTs`（国光「4821900000纸或…」）；余文非空经 `split_target` 发射给 `gname`（#84：OCR 常把编号与品名合成一个字块） |
| `raw_review` | 原文进字段，状态 `needs_review`。申报要素不编 `0\|0\|...` |

## 逐项整形（#84，续行合并后）

标准报关单（全国统一版式）一件商品占三个物理行，复合列按行分布。续行合并后逐项过四条整形规则：

| 规则 | 触发形状 | 落点 |
|---|---|---|
| 粘连品名兜底拆 | `codeTs` 空且 `gname` 以 8–10 位 HS 开头（粘块落到名称列） | 拆 `codeTs` + `gname` |
| 法定/成交数量拆 | `gqty` 值带单位后缀（`48千克`）**且** `gunit` 值是「数字+单位」（`240盒`） | 重量单位方 → `qty1`/`unit1`（unit1 千克时同值补 `customNetWt`）；另一方 → `gqty`/`gunit`。裸数字+纯单位堆叠（当纳利）形状不符不拆 |
| 境内目的地双码 | 值前缀 `（44536／440308）` 两码都非空 | 第一码 `districtCode`、第二码 `ciqDestCode`（新字段）；单码前缀（当纳利 `(44199/)东莞/`）保持原值 |
| HS `split_target` | 编号列值 `1905310000半岛…`（粘块落到编号列） | HS 留 `codeTs`、余文发射 `gname` |
